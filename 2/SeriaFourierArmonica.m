function SeriaFourierArmonica(T, nume)
...
for k=1:N
 ...
 % Determinarea coeficientilor seriei Fourier armonice
 A(k+1) = sqrt(a(k)^2+b(k)^2);
 if a(k)==0 && b(k)==0
 % daca ambii coeficienti ak si bk sunt 0 atunci si faza va fi 0
 fi(k+1) = 0;
 else
 % Determinam faza in cele doua cazuri
 if a(k)>=0
